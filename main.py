...
class F1Plugin(Star):
    ...
    async def _maybe_auto_update_standings_with_timeout(self, timeout_sec: float = 3.0) -> str | None:
        try:
            return await asyncio.wait_for(self._maybe_auto_update_standings(), timeout=timeout_sec)
        except asyncio.TimeoutError:
            logger.warning(f"[F1Plugin] auto update standings timeout after {timeout_sec}s")
            return None
    ...
    def _load_news_articles(self, limit: int = 5) -> list[dict]:
        news_path = self.project_root / "f1_paddock_news.json"
        ...
        try:
            ...
            logger.warning(f"[F1Plugin] read paddock news failed: {e}")
            return []

    @staticmethod
    def _light_translate_en_to_zh(text: str) -> str:
        """轻量术语翻译：不依赖第三方翻译服务，仅做常见 F1 英文词替换。"""
        if not text:
            return ""
        translated = text
        glossary = {
    ...
    def _get_news_update_status_text(self) -> str:
        news_path = self.project_root / "f1_paddock_news.json"
        if not news_path.exists():
            return "📰 新闻更新状态：未生成新闻缓存文件"

        try:
            with open(news_path, "r", encoding="utf-8") as f:
                data = json.load(f)
        except Exception as e:
            logger.warning(f"[F1Plugin] read paddock news status failed: {e}")
            return "📰 新闻更新状态：读取失败"

        warning = str(data.get("warning", "")).strip()
        articles = data.get("articles", [])
        if warning:
            return f"📰 新闻更新状态：已跳过（{warning}）"
        if isinstance(articles, list):
            return f"📰 新闻更新状态：成功（{len(articles)} 条）"
        return "📰 新闻更新状态：格式异常"

    @staticmethod
    def _split_command_and_arg(raw_message: str) -> tuple[str, str]:
    ...
    @filter.command("f1ping")
    async def f1ping(self, event: AstrMessageEvent):
        """F1 插件探针命令。"""
        yield event.plain_result("✅ F1 插件在线")

    @filter.command("f1help")
    ...
    async def f1update(self, event: AstrMessageEvent):
        ...
        try:
            ...
            yield event.plain_result(self._get_news_update_status_text())
        except Exception as e:
            logger.exception(e)
            yield event.plain_result(f"❌ 数据更新失败: {e}")

    @filter.command("f1driver")
    async def f1driver(self, event: AstrMessageEvent):
        ...
        if not query:
            ...
            auto_msg = await self._maybe_auto_update_standings_with_timeout()
            if auto_msg:
    ...
    @filter.command("f1news")
    async def f1news(self, event: AstrMessageEvent):
        """查看近期围场新闻（来自本地缓存），支持 /f1news N。"""
        raw_arg = self._extract_arg(event.message_str)
        limit = 5
        if raw_arg:
            try:
                limit = int(raw_arg)
            except ValueError:
                yield event.plain_result("❌ 参数必须是数字，例如 /f1news 10")
                return
        ...
        articles = self._load_news_articles(limit=limit)
        if not articles:
            yield event.plain_result("暂无新闻数据，请先执行 /f1update 后再试。")
            return

        lines = [f"📰 近期围场新闻（Top {len(articles)}）"]
        for idx, item in enumerate(articles, start=1):
            title = str(item.get("title", "(无标题)")).strip()
            title_zh = str(item.get("title_zh", "")).strip()
            if not title_zh:
                title_zh = self._light_translate_en_to_zh(title)
            link = str(item.get("link", "")).strip()
            if title_zh and title_zh != title:
                lines.append(f"{idx}. {title_zh}")
    ...
    async def f1status(self, event: AstrMessageEvent):
        ...
        checks = {
            ...
            "f1_paddock_news.json": (self.project_root / "f1_paddock_news.json").exists(),
